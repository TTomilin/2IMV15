...
inline Mat4 operator*(const Mat4& n, const Mat4& m) {
    Mat4 res = Mat4(Vec4(0, 0, 0, 0), Vec4(0, 0, 0, 0), Vec4(0, 0, 0, 0), Vec4(0, 0, 0, 0));
    for (uint8_t x = 0; x < 4; x++) {
        for (uint8_t y = 0; y < 4; y++) {
            res[y][x] = m[y][0] * n[0][x] + m[y][1] * n[1][x] + m[y][2] * n[2][x] + m[y][3] * n[3][x];
        }
    }
    return res;
}
...
inline Mat4 translation_matrix(const Vec3& delta) {
    return Mat4(
        Vec4(1.0, 0.0, 0.0, delta[0]),
        Vec4(0.0, 1.0, 0.0, delta[1]),
        Vec4(0.0, 0.0, 1.0, delta[2]),
