...
public:
    ...
    double magnitude() {
        return sqrt(pow(elt[0], 2) + pow(elt[1], 2) + pow(elt[2], 2));
    }
};
...
/* 
 * Alternative function name for dot product (can also be done using v1*v2)
 */
template<class T> inline T dotProduct(const TVec3<T>& v1, const TVec3<T>& v2)
{
    return v1 * v2;
}
...
//template<class T> inline Mat3 outer(const Vec3& a, const Vec3& b)
//{
//    return Mat3(
...
template<class T> inline TVec3<T> rotate(const TVec3<T>& v, const TVec3<T>& axis, const T angle)
{
    // Rodrigues rotation formula
    return v * cos(angle) +(cross(axis, v) * sin(angle) + axis * (dotProduct(axis, v) * (1.0f - cos(angle))));
}
